...
- `/`：校园集市介绍页，使用真实校园/商品素材与轻量动效；访客从这里进入登录或注册。
- `/login`：互动角色登录页；从介绍页登录按钮进入时有颜色圆点转场，角色自身输入联动保持独立。
- `/register`：邮箱验证码注册；成功后进入四步资料引导。
- `/app`：校园交易主界面；左侧导航使用距离感应的文字动效，信任卡按用户分数显示从暖色到绿色的等级色。
- 资料引导保存接口：`PUT /marketplace/profile`，会同步头像、学校、签名、背景和主题到现有个人资料数据。
