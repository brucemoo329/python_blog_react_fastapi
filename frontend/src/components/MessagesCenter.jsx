  ...
  const loadConversations = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setActiveId((current) => {
        ...
        return initialIdRef.current || items[0]?.id || null
      })
          ...
          <>
            <header className="messages-thread-header">
              ...
              <div className="messages-thread-actions">
                <Button variant="ghost" size="sm" onClick={clearChat}><Eraser /> 清空记录</Button>
                <Button variant="ghost" size="sm" onClick={removeConversation}><Trash2 /> 删除会话</Button>
              </div>
            </header>
            <ChatThread
              conversation={active}
